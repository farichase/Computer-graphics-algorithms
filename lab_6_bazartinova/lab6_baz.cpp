...
void drawWithAnim() {
    ...
    while (indexZ < coordinatesZ.size()) { //рисую tube
        ...
        while (index < vertexesVertical.size()){
            ...
            glTexCoord2f(lastX / radius, coordinatesZ[indexZ - 1] / height);
            glNormal3f(-lastX, lastY, 0);
            glVertex3f(bx, by, bz);  
            bx = calcul(x, x);
            by = calcul(y, y);
            bz = calcul(coordinatesZ[indexZ - 1], -coordinatesZ[indexZ - 1]);
            glTexCoord2f(x / radius, coordinatesZ[indexZ - 1] / height);
            glNormal3f(-x, -y, 0);
            glVertex3f(bx, by, bz);
            bx = calcul(x, x);
            by = calcul(y, y);
            bz = calcul(coordinatesZ[indexZ], -coordinatesZ[indexZ]);
            glTexCoord2f(x / radius, coordinatesZ[indexZ] / height);
            glNormal3f(-x, y, 0);
            glVertex3f(bx, by, bz);
            bx = calcul(lastX, lastX);
            by = calcul(lastY, lastY);
            bz = calcul(coordinatesZ[indexZ], -coordinatesZ[indexZ]);
            glTexCoord2f(lastX / radius, coordinatesZ[indexZ] / height);
            glNormal3f(-lastX, -lastY, 0);
            glVertex3f(bx, by, bz);
        ...
        while (index1 < vertexesBase.size()){
            ...
            while(index2 < vertexesBase[0].size()){
                ...
                if (!IS_TEXTURE){
                    ...
                    bz = calcul(step, -step);
                    glVertex3f(bx, by, bz);
                    glTexCoord2d(lastX / radius, lastY / radius); 
                    bx = calcul(lastX, lastX);
                    by = calcul(lastY, lastY);
                    bz = calcul(step, -step);
                    glVertex3f(bx, by, bz);
